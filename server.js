...
app.post('/api/work-hours', auth, async (req, res) => {
  ...
  try {
    ...
    if(!date || !h || h<=0 || h>24) return res.status(400).json({error:'G\u00fcltige Stunden (0-24) erforderlich'});
    const r = await pool.query(
      'INSERT INTO work_hours (date,hours,note,created_by) VALUES ($1,$2,$3,$4) RETURNING *',
      [date, h, note||'', req.user.email]
    );
    res.json(r.rows[0]);
  } catch(e){ res.status(500).json({error:e.message}); }
});
