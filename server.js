...
function buildEInvoiceXml(inv, firm) {
  ...
  const rLines = String(inv.recipient||'').split('\n').map(l=>l.trim()).filter(Boolean);
...
app.post('/api/customers', auth, async (req, res) => {
  if(req.user.role !== 'admin') return res.status(403).json({error:'Nur Admin'});
  try {
    const {name, address, email, ust_id} = req.body;
    if(!name || !name.trim()) return res.status(400).json({error:'Name erforderlich'});
    const existing = await pool.query('SELECT id FROM customers WHERE name=$1', [name.trim()]);
    let r;
    if(existing.rows.length) {
      r = await pool.query("UPDATE customers SET address=$1, email=COALESCE(NULLIF($2,''),email), ust_id=COALESCE(NULLIF($3,''),ust_id) WHERE id=$4 RETURNING *",
        [address||'', email||'', ust_id||'', existing.rows[0].id]);
    } else {
      r = await pool.query('INSERT INTO customers (name, address, email, ust_id) VALUES ($1,$2,$3,$4) RETURNING *',
        [name.trim(), address||'', email||'', ust_id||'']);
    }
    res.json(r.rows[0]);
  } catch(e){ res.status(500).json({error:e.message}); }
});
...
app.put('/api/customers/:id', auth, async (req, res) => {
  ...
  try {
    const {name, address, email, ust_id} = req.body;
    const r = await pool.query('UPDATE customers SET name=$1, address=$2, email=$3, ust_id=$4 WHERE id=$5 RETURNING *',
      [name||'', address||'', email||'', ust_id||'', req.params.id]);
