...
// ── MODELLE ──
app.get('/api/models', auth, async (req, res) => { const r = await pool.query('SELECT name, COALESCE(price,0) as price, COALESCE(preassembled,false) as preassembled FROM models ORDER BY name'); res.json(r.rows); });
...
app.put('/api/models/preassembled', auth, async (req, res) => { if(!canWrite(req)) return res.status(403).json({error:'Kein Schreibrecht'}); const {name, preassembled} = req.body; await pool.query('UPDATE models SET preassembled=$1 WHERE name=$2', [preassembled===true||preassembled==='true', name]); res.json({ok:true}); });
...
app.get('/api/work-hours', auth, async (req, res) => {
  try {
    ...
    else r = await pool.query('SELECT * FROM work_hours WHERE created_by=$1 ORDER BY date DESC, created_at DESC', [req.user.email]);
    res.json(r.rows);
  } catch(e){ res.status(500).json({error:e.message}); }
});
...
app.post('/api/work-hours', auth, async (req, res) => {
  ...
  try {
    ...
    if(req.body.employee) employee = String(req.body.employee);
    const r = await pool.query(
...
// ===== Mitarbeiter-Stammliste (Namen, unabh\u00e4ngig vom Login; f\u00fcr Zeiterfassung) =====
app.get('/api/employees', auth, async (req, res) => {
  try { const r = await pool.query('SELECT * FROM employees ORDER BY active DESC, name'); res.json(r.rows); }
  catch(e){ res.status(500).json({error:e.message}); }
});
app.post('/api/employees', auth, async (req, res) => {
  ...
  try {
    ...
    const r = await pool.query('INSERT INTO employees (name,active) VALUES ($1,true) RETURNING *', [name]);
    res.json(r.rows[0]);
  } catch(e){ res.status(500).json({error:e.message}); }
});
app.put('/api/employees/:id', auth, async (req, res) => {
  ...
  try {
    ...
    if(!name) return res.status(400).json({error:'Name erforderlich'});
    const r = await pool.query('UPDATE employees SET name=$1, active=$2 WHERE id=$3 RETURNING *', [name, active, req.params.id]);
