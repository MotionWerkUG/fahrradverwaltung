...
const pool = new Pool({
  ...
  password: process.env.DB_PASS || 'SanduMotion2025',
...
async function generateInvoicePDF(inv, s) {
  return generatePDF(doc => {
    ...
    // Firmenname + Rechtsform oben links (kein brauner Balken)
    ...
    doc.fontSize(8).font('Helvetica').fillColor('#999')
       .text((s.strasse||'')+' | '+(s.ort||'')+(s.email?' | '+s.email:''), 50, y);
    y += 6;
    ...
    recipLines.forEach((line, i) => {
      if(i===0) doc.fontSize(11).font('Helvetica-Bold').fillColor('#222').text(line, 50, y);
      else doc.fontSize(10).font('Helvetica').fillColor('#333').text(line, 50, y);
      y += 14;
    });

    // Rechnungsinfo rechts
    doc.fontSize(15).font('Helvetica-Bold').fillColor(beige)
       .text('Rechnung '+inv.nr, 300, 42, {align:'right', width:245});
    doc.fontSize(9.5).font('Helvetica').fillColor('#555')
       .text('Rechnungsdatum: '+inv.date, 300, 64, {align:'right', width:245})
       .text('Leistungsdatum: '+(inv.service_date||inv.date), 300, 78, {align:'right', width:245});

    // Tabellen-Header dunkelgrau
    y = Math.max(y+8, 130);
    doc.fillColor('#444').rect(50,y,495,22).fill();
    doc.fillColor('white').fontSize(9).font('Helvetica-Bold');
    doc.text('Pos.', 58, y+7);
    doc.text('Beschreibung', 90, y+7);
    ...
    // Positionen
    items.forEach((it, i) => {
      const rowH = it.description ? 28 : 18;
      if(i%2===1) doc.fillColor('#f7f7f7').rect(50,y,495,rowH).fill();
      doc.fillColor('#333').fontSize(9.5).font('Helvetica');
      doc.text(String(i+1), 58, y+5);
      doc.text('Montage '+it.model, 90, y+5, {width:235});
      if(it.description) doc.fontSize(8).fillColor('#888').text(it.description, 90, y+17, {width:235});
      doc.fontSize(9.5).fillColor('#333');
      doc.text(it.qty+' Stk.', 330, y+5, {align:'right', width:65});
      doc.text(Number(it.price).toFixed(2).replace('.',',')+' EUR', 400, y+5, {align:'right', width:70});
      doc.text(Number(it.total).toFixed(2).replace('.',',')+' EUR', 475, y+5, {align:'right', width:65});
      y += rowH;
    });
    ...
    doc.text('Nettobetrag:', 340, y);
    doc.text(Number(inv.netto).toFixed(2).replace('.',',')+' EUR', 475, y, {align:'right',width:65}); y += 17;
    doc.text('zzgl. 19% MwSt:', 340, y);
    doc.text(Number(inv.mwst).toFixed(2).replace('.',',')+' EUR', 475, y, {align:'right',width:65}); y += 7;
    ...
    if(s.bank||s.iban){ doc.text('Bank: '+(s.bank||'')+' | IBAN: '+(s.iban||'')+(s.bic?' | BIC: '+s.bic:''), 50, y, {width:495}); y += 13; }
    ...
    const fcols = [
      ...
      ['Geschäftsführer:', s.gf||'–', s.gericht||''],
    ...
    fcols.forEach((col, ci) => {
      ...
      col.forEach((line, li) => {
        doc.fontSize(7).font(li===0?'Helvetica-Bold':'Helvetica').fillColor('#777')
           .text(line, cx, fy2, {width:118});
        fy2 += 9;
      });
      if(ci<3) doc.moveTo(cx+120,fY+3).lineTo(cx+120,fY+38).strokeColor('#ddd').lineWidth(0.5).stroke();
    });
